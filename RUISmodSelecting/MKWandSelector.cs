...
[System.Serializable]
[AddComponentMenu("MK/Input/MKWandSelector")]
public class MKWandSelector : MonoBehaviour {
    ...
    private MKSelectable selection;
    public MKSelectable Selection
    {
        get
        {
            return selection;
    ...
    private MKSelectable highlightedObject;
    public MKSelectable HighlightedObject
    {
        get
        {
    ...
    public void Awake()
    {
        ...
        if (!wand)
        {
            Debug.LogError(name + ": MKWandSelector requires a RUISWand");
        }
    ...
    public void Update()
    {
        ...
        if (!selection)
        {
            ...
            {
                MKSelectable selectableObject = selectionGameObject.GetComponent<MKSelectable>();
                //also search in parents if we didn't find MKSelectable on this gameobject to allow for multi-piece collider hierarchies
                while (!selectableObject && selectionGameObject.transform.parent != null)
                {
                    ...
                    selectableObject = selectionGameObject.GetComponent<MKSelectable>();
                }
            ...
            if (!selectionGameObject || !selectionGameObject.GetComponent<MKSelectable>())
            {
                if (highlightedObject != null)
                {
                    highlightedObject.OnSelectionHighlightEnd();
...
//        lineRenderer.enabled = selection == null && selectionRayType == SelectionRayType.WandDirection;
		...
		if(selection)
		{
			...
			// if selected object is one of the MKSelectableJoints 
